...
import { FORMAT_FLOWS } from "../create/format-flow";
...
export function renderDashboardHome(model: DashboardHomeModel): string {
  ...
  return `<div class="shell dashboard-shell">
    ...
    <dialog class="format-dialog" data-format-dialog aria-labelledby="format-dialog-title"><div class="format-dialog__header"><div><p class="format-dialog__eyebrow">Choisis un point de départ</p><h2 id="format-dialog-title">Nouvelle page</h2><p>Pars d’un format adapté à ce que tu veux créer.</p></div><button class="format-dialog__close" type="button" data-format-dialog-close aria-label="Fermer la sélection de format">×</button></div><div class="format-card-grid">${FORMAT_FLOWS.map(formatCard).join("")}</div></dialog>
