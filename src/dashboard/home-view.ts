...
export function renderDashboardHome(model: DashboardHomeModel): string {
  ...
  return `<div class="shell dashboard-shell">
    <aside class="sidebar" aria-label="Navigation principale">
      ...
      <nav class="main-nav"><a class="nav-item is-active" href="/dashboard"><span>⌂</span>Accueil</a><a class="nav-item" href="#creations"><span>▣</span>Mes créations <b>${model.totalProjects}</b></a><a class="nav-item" href="/facturation#shopify"><span>◆</span>Ma boutique</a><a class="nav-item" href="/facturation"><span>◈</span>Abonnement</a></nav>
    ...
    <main><header class="topbar"><div><p class="hello">${escapeHtml(model.greeting)}</p><p class="subhello">Transforme ton prochain produit en boutique.</p></div><a class="pro-button" href="/facturation">Passer Pro</a></header>
      <section class="creation-desk"><div class="duck" aria-hidden="true">🐥</div><div class="desk-copy"><h1>Que veux-tu vendre ?</h1><p>Donne-moi un produit. Je m’occupe de l’offre, des mots et de la boutique.</p></div>
        ...
        <div class="start-modes"><button data-dashboard-action="link"><span>↗</span><strong>Importer un lien</strong><small>Produit et images</small></button><button data-dashboard-action="image"><span>▧</span><strong>Ajouter une image</strong><small>On reconnaît le produit</small></button><button data-dashboard-action="shopify"><span class="shopify-mark">S</span><strong>Depuis Shopify</strong><small>Choisir dans le catalogue</small></button><button data-dashboard-action="blank"><span>＋</span><strong>Partir de zéro</strong><small>Une page vraiment vierge</small></button></div>
      </section>
      <section class="projects" id="creations"><div class="section-heading"><div><h2>Mes créations</h2><p>Reprends là où tu t’es arrêté.</p></div><button data-dashboard-action="all">Tout afficher →</button></div><div class="project-shelf">${cards}</div></section>
      <section class="workbench"><div class="next-actions"><div class="section-heading compact"><div><h2>Le prochain geste</h2><p>Publie sans casser ton thème Shopify.</p></div></div><button class="task-row" data-dashboard-action="shopify"><span class="task-icon">S</span><span><strong>Connecter Shopify</strong><small>Choisir le thème au moment de publier</small></span><b>Configurer →</b></button><button class="task-row" data-dashboard-action="generate"><span class="task-icon">Aa</span><span><strong>Créer une nouvelle offre</strong><small>Canardo adapte le message au produit</small></span><b>Commencer →</b></button></div>
        <aside class="shopify-card"><span class="shopify-mark large">S</span><p class="mini-title">Publication Shopify</p><h2>Ta boutique, dans ton vrai thème.</h2><p>Sections modifiables, copie sécurisée et retour arrière inclus.</p><button data-dashboard-action="shopify">Connecter ma boutique</button></aside></section>
    </main></div>
    <nav class="mobile-nav"><a href="/dashboard">⌂<span>Accueil</span></a><a href="#creations">▣<span>Créations</span></a><button data-dashboard-action="generate">＋<span>Créer</span></button><a href="/facturation#shopify">◆<span>Shopify</span></a><a href="/facturation">⚙<span>Réglages</span></a></nav>`;
}
