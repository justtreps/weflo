...
export const productHeroSection = {
  ...
  renderWeb: ({ section, pageName }: Parameters<typeof base.renderWeb>[0]) => {
    ...
    if (variant === "problem-solution") return `<section class="wf-section wf-hero wf-hero__problem" data-wf-variant="problem-solution"><div class="wf-hero__problem-copy">${subtitle ? `<span class="wf-section__eyebrow">${escapeHtml(subtitle)}</span>` : ""}${edit("h1","title",title)}${edit("p","text",body)}${price ? edit("strong","price",price,"wf-section__price") : ""}${action}</div><figure>${media}</figure></section>`;
