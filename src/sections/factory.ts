...
function renderBlocks(blocks: EditorBlock[], tag = "article"): string {
  return blocks.map((block) => { const rating = Number(block.settings.rating); return `<${tag} class="wf-section__card" data-wf-block-id="${escapeHtml(block.id)}">${block.settings.image ? `<img src="${safeMediaUrl(block.settings.image)}" alt="${escapeHtml(blockValue(block, "image_alt", blockValue(block, "title")))}">` : ""}${Number.isFinite(rating) && rating > 0 ? `<span role="img" aria-label="${rating} étoiles sur 5">${"★".repeat(Math.min(5, rating))}</span>` : ""}<h3>${escapeHtml(blockValue(block, "title", blockValue(block, "label", "Élément")))}</h3><p>${escapeHtml(blockValue(block, "text"))}</p>${block.settings.link ? `<a href="${safeLink(block.settings.link)}">${escapeHtml(blockValue(block, "label", "Découvrir"))}</a>` : ""}</${tag}>`; }).join("");
}
