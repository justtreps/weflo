...
import { mergeGenerations, readCachedGenerations } from "../studio/browser-history";
import "./studio.css";
import "./studio-pending.css";
...
async function start() {
  ...
  let history = await api<History>("/api/studio/generations").catch(() => ({ generations: [] })); history.generations = mergeGenerations(cached, history.generations); let model: ImageModelId = "nano-banana-2"; let ratio: ImageAspectRatio = "1:1"; let count = 1; let referenceUrl: string | null = null; let insertUrl = ""; let pending: { prompt: string; model: ImageModelId; aspectRatio: ImageAspectRatio; numImages: number } | null = null;
  const render = () => { root.innerHTML = renderStudioView({ workspaceName: me.workspace.name, generations: history.generations, pending }); bind(); };
  ...
  const bind = () => {
    ...
    root.querySelector<HTMLFormElement>("[data-studio-form]")?.addEventListener("submit", async (event) => { event.preventDefault(); const prompt = root.querySelector<HTMLTextAreaElement>("textarea")?.value.trim() ?? ""; if (!prompt || pending) return; pending = { prompt, model, aspectRatio: ratio, numImages: count }; const request = { model, aspectRatio: ratio, numImages: count, referenceUrl }; render(); root.querySelector<HTMLElement>("[data-generation-pending]")?.scrollIntoView({ behavior: "smooth", block: "center" }); try { const generated = await api<ImageGeneration>("/api/studio/generate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ prompt, ...request }) }); history = { generations: mergeGenerations([generated], history.generations) }; localStorage.setItem(cacheKey, JSON.stringify(history.generations)); pending = null; render(); root.querySelector<HTMLElement>(`[data-generation-id="${generated.id}"]`)?.scrollIntoView({ behavior: "smooth", block: "center" }); } catch (error) { pending = null; render(); const grid = root.querySelector<HTMLElement>("[data-result-grid]"); if (grid) grid.insertAdjacentHTML("afterbegin", `<div class="studio-error"><div><h2>La génération n’a pas abouti.</h2><p>${error instanceof Error ? error.message : "Réessaie dans un instant."}</p></div></div>`); } });
    root.querySelectorAll<HTMLElement>("[data-image-command]").forEach((button) => button.addEventListener("click", async () => { const url = button.dataset.imageUrl ?? ""; if (button.dataset.imageCommand === "reference" || button.dataset.imageCommand === "variation") { referenceUrl = url; paintReference(); root.querySelector("textarea")?.focus(); } if (button.dataset.imageCommand === "insert") { insertUrl = url; const dialog = root.querySelector<HTMLDialogElement>("[data-insert-dialog]")!; const pages = await api<Pages>("/api/pages"); const choices = dialog.querySelector<HTMLElement>("[data-page-choices]")!; choices.innerHTML = pages.pages.length ? pages.pages.map((page) => `<button data-page-id="${page.id}">${page.name}<small> · ${page.type === "sell" ? "Page produit" : "Page"}</small></button>`).join("") : `<a href="/creer">Créer une page d’abord</a>`; choices.querySelectorAll<HTMLElement>("[data-page-id]").forEach((choice) => choice.addEventListener("click", () => { const pageId = choice.dataset.pageId!; sessionStorage.setItem("weflo-studio-insert", JSON.stringify({ pageId, imageUrl: insertUrl })); location.assign(`/editeur?page=${pageId}`); })); dialog.showModal(); } }));
