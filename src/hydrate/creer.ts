...
async function startFromAnswers() { const body = await request("/api/onboarding/start", { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({ creationFormat:state.format, templateId:state.templateId, answers:state.answers, prompt:state.prompt, language:"fr" }) }); draft=body.draft;token=body.claimToken; }
async function loadShopifyProducts(){shopifyCatalog={...shopifyCatalog,status:"loading",message:""};render();try{const body=await request("/api/shopify/products",{method:"GET"});shopifyCatalog={...shopifyCatalog,status:"ready",products:Array.isArray(body.products)?body.products:[],message:""};}catch(reason){shopifyCatalog={...shopifyCatalog,status:"unavailable",products:[],message:reason instanceof Error?reason.message:"Impossible de charger le catalogue Shopify. Réessaie."};}render();}
async function importShopifyProduct(productId:string){const product=shopifyCatalog.products.find((item)=>item.id===productId);if(!product)return;const form=root?.querySelector<HTMLFormElement>("[data-source-form]");if(form){const data=new FormData(form);state=transitionCreationFlow(state,{type:"UPDATE_INTAKE",answers:answersFromFormData(data),prompt:String(data.get("prompt")??"")});persistState();replaceWorkspaceUrl();}shopifyCatalog={...shopifyCatalog,status:"loading",selectedId:productId,selectedTitle:product.title,message:""};render();try{const body=await request("/api/onboarding/import-shopify",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({productId,language:"fr"})});draft=body.draft;token=body.claimToken;shopifyCatalog={...shopifyCatalog,status:"ready",selectedId:productId,selectedTitle:product.title,message:""};}catch(reason){shopifyCatalog={...shopifyCatalog,status:"unavailable",message:reason instanceof Error?reason.message:"Impossible d’importer ce produit Shopify. Réessaie."};}render();}
...
function bind() {
  root?.querySelectorAll<HTMLElement>("[data-create-format]").forEach((button)=>button.addEventListener("click",()=>{missingFieldIds=[];draft=null;shopifyCatalog={status:"idle",products:[],message:"",selectedId:null,selectedTitle:""};commitState(transitionCreationFlow(state,{type:"SELECT_FORMAT",format:button.dataset.createFormat as CreationFlowState["format"] & string}));if(state.step==="create-blank")void createBlankPage();}));
  ...
  intakeForm?.addEventListener("submit",async(event)=>{
    ...
    if(action==="shopify"&&!draft){submissionLock.release();shopifyCatalog={...shopifyCatalog,status:"unavailable",message:"Choisis un produit du catalogue Shopify avant de continuer."};render();return;}
    render();
...
window.addEventListener("popstate",(event)=>{const url=new URL(location.href);const historyDraft=event.state&&typeof event.state.wefloCreationDraft==="string"?restoreCreationDraft(event.state.wefloCreationDraft):null;state=historyDraft??mergeCompatibleCreationDraft(initialCreationState(url),readSavedState(),url);persistState();replaceWorkspaceUrl();if(creationStartupAction(state)==="create-blank")void createBlankPage();else render();});
