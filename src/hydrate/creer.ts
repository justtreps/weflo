...
import { onboardingDraftPatch } from "../create/onboarding-sync";
import "./creer.css";
...
async function syncDraft(includeStrategy = false) { if (!draft) return; const strategy = includeStrategy ? { personas:draft.personas, angles:draft.angles } : undefined; await request(`/api/onboarding/${draft.id}`, { method:"PATCH", headers:{"content-type":"application/json","x-weflo-claim-token":token}, body:JSON.stringify(onboardingDraftPatch(state,strategy)) }); }
