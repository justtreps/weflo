...
async function request(url: string, init: RequestInit) { const response = await fetchWithDeadline(url,init); const body = await readApiJson(response); if (!response.ok) throw new Error(body.message || "Cette étape n’a pas abouti."); return body; }
...
function bind() {
  ...
  intakeForm?.addEventListener("submit",async(event)=>{
    ...
    render();
    try{error="";if(action==="link")await importLink(updated.prompt);if(action==="image")await syncDraft();if(action==="simple")await startFromAnswers();submissionLock.release();commitState(transitionCreationFlow(updated,{type:"CONTINUE"}));}catch(reason){submissionLock.release();if(state.step==="strategy"&&!draft)state=transitionCreationFlow(state,{type:"BACK"});persistState();replaceWorkspaceUrl();error=reason instanceof Error?reason.message:"Import impossible";render();}
  });
...
window.addEventListener("popstate",(event)=>{const url=new URL(location.href);const historyDraft=event.state&&typeof event.state.wefloCreationDraft==="string"?restoreCreationDraft(event.state.wefloCreationDraft):null;state=historyDraft??mergeCompatibleCreationDraft(initialCreationState(url),readSavedState(),url);persistState();replaceWorkspaceUrl();if(creationStartupAction(state)==="create-blank")void createSimple().catch((reason)=>{error=reason instanceof Error?reason.message:"Création impossible";render();});else render();});
