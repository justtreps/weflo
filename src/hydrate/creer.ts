...
async function createSimple() { const type = state.format === "blog" ? "write" : state.format === "blank" ? "blank" : "sell"; const name = state.prompt.trim() || creationFormats.find((item) => item.id === state.format)?.title || "Nouvelle page"; const page = await request("/api/pages", { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({ type, name }) }); location.assign(`/editeur?page=${page.id}`); }
async function build() { if (!draft) return; state=transitionCreationFlow(state,{type:"START_BUILD"});persistState();replaceWorkspaceUrl();busy=true;error="";buildStageIndex=0;renderBuild();let timer:ReturnType<typeof setInterval>|undefined;try{await request(`/api/onboarding/${draft.id}`,{method:"PATCH",headers:{"content-type":"application/json","x-weflo-claim-token":token},body:JSON.stringify({creationFormat:state.format??"store",personas:draft.personas,angles:draft.angles,language:"fr"})});buildStageIndex=1;renderBuild();timer=setInterval(()=>{if(!draft)return;const ceiling=Math.max(1,draft.stages.length-2);if(buildStageIndex<ceiling){buildStageIndex+=1;renderBuild();}},650);await request(`/api/onboarding/${draft.id}/build`,{method:"POST",headers:{"x-weflo-claim-token":token}});if(timer)clearInterval(timer);timer=undefined;while(buildStageIndex<draft.stages.length-1){buildStageIndex+=1;renderBuild();await new Promise((resolve)=>setTimeout(resolve,90));}const claimed=await request(`/api/onboarding/${draft.id}/claim`,{method:"POST",headers:{"x-weflo-claim-token":token}});await new Promise((resolve)=>setTimeout(resolve,450));location.assign(`/editeur?page=${claimed.pageId}`);}finally{if(timer)clearInterval(timer);} }
function bind() {
