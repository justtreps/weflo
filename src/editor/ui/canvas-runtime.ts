export const CANVAS_RUNTIME = `<style>.wf-canvas-toolbar{position:absolute;z-index:9999;display:flex;gap:2px;padding:3px;border-radius:7px;background:#141310;color:#fff;transform:translateY(-100%)}.wf-canvas-toolbar button{border:0;padding:6px 8px;background:transparent;color:inherit;font:600 11px/1 sans-serif;cursor:pointer}[data-wf-section-id]{position:relative}[data-wf-section-id][draggable="true"]{cursor:grab}[data-wf-edit-key][contenteditable="true"]{outline:2px solid #315efb;outline-offset:3px}</style><script>(()=>{
  ...
  let dragging=null;
  const toolbar=(section,imageKey)=>{document.querySelector("[data-canvas-toolbar]")?.remove();const bar=document.createElement("div");bar.className="wf-canvas-toolbar";bar.dataset.canvasToolbar="";bar.innerHTML=(imageKey?'<button data-canvas-action="editImage">✦ Edit with AI</button>':'')+'<button data-canvas-action="moveUp">↑</button><button data-canvas-action="moveDown">↓</button><button data-canvas-action="duplicate">Duplicate</button><button data-canvas-action="hide">Hide</button><button data-canvas-action="remove">Delete</button>';bar.addEventListener("click",e=>{const action=e.target.closest("[data-canvas-action]")?.dataset.canvasAction;if(action==="editImage")post("canvas:image-edit",{sectionId:section.dataset.wfSectionId,key:imageKey});else if(action)post("canvas:action",{sectionId:section.dataset.wfSectionId,action})});section.prepend(bar)};
