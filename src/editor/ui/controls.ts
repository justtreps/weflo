...
export function inspectorControlMarkup(section: EditorSection, control: InspectorControl, breakpoint: "desktop" | "tablet" | "mobile"): string {
  ...
  const optionLabels: Record<string, string> = { left: "Gauche", center: "Centre", right: "Droite", none: "Aucune", fade: "Fondu", reveal: "Révélation" };
  if (control.type === "select") return `<label class="editor-control"><span>${control.label}</span><select ${attrs}>${control.options?.map((option) => `<option value="${option}"${value === option ? " selected" : ""}>${optionLabels[option] ?? option}</option>`).join("")}</select></label>`;
  ...
  return `<label class="editor-control"><span>${control.label}</span><input type="${inputType}" ${attrs} value="${escape(value)}">${control.type === "image" && value ? `<button type="button" class="editor-image-ai" data-image-ai data-image-key="${control.key}">✦ Modifier avec l’IA</button>` : ""}</label>`;
}
